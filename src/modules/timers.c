...
RCC->APB1ENR |= RCC_APB1ENR_TIM3EN;   //подать тактирование на TIM3                           /
   ...
   TIM3->PSC = 1999;          // предделитель таймера 2
   TIM3->ARR = 99;      //
   ...
   TIM3->CR1 |= TIM_CR1_CEN;       //запускаем таймер2
...
void TIM2_IRQHandler(void)
{
  static u16 InsideCounter = 0;
   ...
   if (TIM2->SR & TIM_SR_UIF){
      ...
      lcd_set_xy(1,0);
      Temper = bmp085_read_temperature();
   ...
   switch (SetCondition){
     ...
     case 1:
         ...
         if(TestMinusButton(TempsLimits)||TestPlusButton(TempsLimits)){
            lcd_set_xy(1,1); DisplayTemperature(TempsLimits[0]);
     ...
     case 2:
         if(TestMinusButton(&TempsLimits[1])||TestPlusButton(&TempsLimits[1])){
            lcd_set_xy(9,1); DisplayTemperature(TempsLimits[1]);
